...
public class formulae extends Applet implements ItemListener,ActionListener,MouseListener,MouseMotionListener
	...
	public void init(){          
	    ...
	    triangle=getImage(base,"triangle.gif");
	    quadrilateral=getImage(base,"quadrilateral.gif");
	   circle=getImage(base,"circle.gif");
	 ...
	 public void paint(Graphics g)
	   {  MediaTracker mt;
	    mt = new MediaTracker(this);
		 ...
		 g.drawString("B",42,274);  g.drawString("(x2, ",33,290);  g.drawString("y2 )",56,290);
	  ...
	  g.drawString("D",490,307);g.drawString("(x4, ",495,317);  g.drawString("y4)",524,317);
  	  ...
  	  g.drawString("CYLINDER",1180,330);
  	    ...
  	    { //triangle formulae
  	      ...
  	      g.drawString(""+distance,20,360);
  	      area="2) Area of the triangle= area^2= (s*(s-a)*(s-b)*(s-c))";
  	    g.drawString(""+area,20,380);
  	  g.drawString("  where s=(a+b+c)/2",50,400);
  	  perimeter="3) Perimeter of triangle= l(AB)+l(BC)+l(CA)";
      g.drawString(""+perimeter,20,420);
    equations="4)Equation of line AB is y = mx + c";
    g.drawString(""+equations,20,440);
    g.drawString("    where m=slope and c=y-Intercept",20,460);
    
    centroid="5)Centroid(xc,yc) is xc=(x1+x2+x3)/3 yc=(y1+y2+y3)/3";
    g.drawString(""+centroid,20,480);
    altitudes="6) In geometry, an altitude of a triangle is a straight line";
    g.drawString(""+altitudes,20,520);
    
    String n="   through a vertex and perpendicular to a line containing";
    g.drawString(""+n,20,540);
    g.drawString("   the base (the opposite side of the triangle).",20,560);
    
g.drawString("7) An angle bisector divides the angle into",20,600);
g.drawString("   two angles with equal measures",20,620);
g.drawString("8) An length bisector divides the length of",20,660);
g.drawString("   the two sides with equal measures",20,680);
